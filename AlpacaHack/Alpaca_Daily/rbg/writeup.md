...
Here, the exponent is generated by the LCG:
...
$$
0 \le e_i \le N-1, \quad 0 \le e_{i+1} \le N-1,
$$
...
We still don’t know \(X\), so we want to eliminate it. Consider the ratio of two consecutive right-hand sides:
...
So once we have a candidate \(Y\), we can enumerate candidates of \(X\) using the small range of \(k_i\).
...
This is not directly \(m\). Instead, raise them to powers \(a\) and \(b\):
...
We can obtain such \(a,b\) via the extended Euclidean algorithm on \(1337\) and \(N\).
